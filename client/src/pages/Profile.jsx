    ...
    uploadTask.on(
      ...
      (error) => {
       ...
       () => {
        ...
        // Update Redux store immediately after upload
); 
      }
    );
  };
...
const handleDeleteUser=async() =>{
 ...
 if (data.success === false) {
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input
          type="email"
          placeholder="email"
          defaultValue={currentUser.email} // Fixed: Now correctly shows email
          id="email"
          className="border p-3 rounded-lg"
          onChange={handleChange}
        />
   ...
   <button onClick={handleShowListings}className="text-green-500 w-full">Show listings</button>
<p className="text-red-700 mt-5">{showListingsError ? 'Error showing listings':''}</p>
     ...
     className="border rounded-lg p-3 flex
      ...
      <div className="flex flex-col items-center">
       <button  onClick={()=>handleListingDelete(listings._id)}className="text-red-600 uppercase">Delete</button>
